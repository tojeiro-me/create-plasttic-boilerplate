...
export default async function starterCli() {
  ...
  const override = await checkFolder(targetDir, ANSWERS.name);
  if (!override) {
    console.log(chalk.red('Exit!'));
    return;
  }
  spinner = ora(`Downloading ${PROJECT} to the ${ANSWERS.name} folder`).start();
...
const TEMPLATES = [
  `${LOCATION}/plasttic-boilerplate`,
  `${LOCATION}/plasttic-boilerplate/boilerplate.html`,
];
...
/** Check if folder exists and ask if to override */
